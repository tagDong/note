...
场景1：如果是5个节点的group在初始化发起投票时，5个节点同时发起了投票，这样选票就无法收到来自其他节点的投票了，这样节点会再次将term加1发起投票，也很有可能会遇到相同状况。
raft内部维持了一个election timeout时间，这个时间是一个范围内的随机值，所以在很大概率上每个节点选举超时的时间是不一致的，这也基本上能解决vote split的问题，当然这个并不能完全解决，但这种随机超时时间基本上可以解决vote split的问题了。
这个election timeout时间范围肯定至少要比网络RTT要大一个数量级，比机器故障时间小一个数量级。broadcastTime << electionTimeout << MTBF。
broadcastTime也即是RTT， MTBF是一台server平均故障时间间隔
场景2：如果当前group中的leader挂掉了，那么这个group中的节点必须要等到election timeout才会发起新的一轮选举，而在新的leader被选举出来之前这个group是不能提供服务的，这样就会造成group的可用性降低。
目前raft论文中并没有针对这期间的可用性问题进行优化。极端情况下这个不可用时间是election timeout。
场景3：网络分区分为对称网络分区与非对称网络分区，三个节点A,B,C，节点A是leader，A与B,C网络分区，这种是对称网络分区，如果A与B网络正常，与C网络分区，但是B与C之间网络正常，这种属于对称网络分区。
A,B,C三个节点，A是leader，B，C是follower
leader与follower出现对称网络分区


这时候A联系不到B,C，会一直不断的尝试发送心跳。
follower侧在一段时间没有接收到来自leader的心跳后，会触发election timeout，然后自增自己的term，变更为candidate，然后向其他节点发起投票。上面说过，这个election timeout是一个区间内的随机值，所以B,C会在不同时间触发election timeout。假设B先触发选举，C在接收到RequestVote RPC之后，与自己当前的term和log比对，发现term和log比自己要新，就会grant这次选举。因为节点中的配置信息是3个节点，B接收到投票时候，符合多数派原则，这时候B被选成了leader，可以正常对外提供服务了。
这里有一点：client原来是与A通信的，但是A与其他节点网络分区后，就一直提供不了服务了，那么这时候client需要有一定措施切换节点重试，也就是说client端RPC必须配置超时，并且需要具备重试其他节点的逻辑，否则会一直处于不可用状态。

leader与follower出现非对称网络分区


A，B网络正常，A与C断连，B与C通信正常。这时候A,B可以正常对外提供服务，C则接收不到来自leader的心跳，在election timeout时间之后，C自增term，变更为candidate，然后发起选举。因为C与B是可以正常通信的，这时候B发现有更高term的选举信息，会变更自己的tern信息，然后回复grant，C就可以成为新的leader。这时候再接收到来自leader A的RPC时，B会将自己的tern信息一并发送给leader A，leader A发现B的term更高，就会自己主动stepdown，但是这时候A与新的leader C同样也存在了非对称网络分区。然后又会循环上面的步骤，这就会导致集群一致处于leader变更状态。

follower与其他节点出现非对称分区


C与A,B都网络分区，这时候集群仍然能够正常提供服务，因为大多数节点仍然是正常的，这时候C节点接收不到A发的心跳，在election timeout之后便会自增term，然后发起投票，如果网络分区一直不修复，C的term信息会一直增加。
如果网络分区修复，C重新加到集群中，这时候A，B会接收到C的requestVote RPC，会发现C的term信息比自己的都大，然后A,B会stepdown，然后进入选举状态。这样正常的集群工作就被打断了。
因此raft中推荐一种prevote机制来避免这种降低集群可用性的场景。当一个节点需要发起投票之前需要先进行prevote，prevote会向其他server发送RPC，如果大多数节点允许其选举(对比term和log)，那么这个节点再自增term并发起选举，否则就不自增term。

raft在leader选举中做了一些限制，不仅仅要判断candidate的term信息，同时要对candidate的log的新旧做判断，如果candidate的log不够新（相同term的logindex较小，或者对应logindex位置term小于接收者的term）那么就拒绝当前candidate的投票选举。

raft的strong leader要求所有的op log都是从leader流向follower，这样的设计简化了日志复制流程。
